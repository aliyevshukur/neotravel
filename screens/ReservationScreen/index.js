  ...
  ({
    ...
    const handleButtonPress = () => {
      switch (stageNumber) {
        case 1:
          if (
            reserveFormValues.guests &&
            reserveFormValues.dateRange.startDate &&
            reserveFormValues.dateRange.endDate
          ) {
    ...
    async function checkIfRoomReserved(data) {
      ...
      try {
        ...
        snapshot.forEach((doc) => {
          if (
            (doc.data().startDate >= data.startDate &&
              doc.data().startDate <= data.endDate) ||
            (doc.data().endDate >= data.startDate &&
              doc.data().endDate <= data.endDate) ||
            (data.startDate <= doc.data().startDate &&
