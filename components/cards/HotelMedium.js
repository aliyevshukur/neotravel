...
export const HotelMedium = ({cardInfo, onPress}) => {
    const item = cardInfo || {};
    ...
    return (
        <TouchableOpacity style={styles.container} onPress={onPress}>
            ...
            >
                ...
                <CustomText style={styles.name}>{makeItShort(item.name, 30) || "~"}</CustomText>
                <CustomText style={styles.place}>{makeItShort(item.place, 20) || "~"}</CustomText>
                <CustomText style={styles.price}>{makeItShort(item.currency, 3) || "$"} {item.price || "~"}</CustomText>
                <Rating style={styles.rating} value={item.rating} starColor={'rgba(255, 255, 255, 0.5)'}/>
            </LinearGradient>
        </TouchableOpacity>
    );
};
