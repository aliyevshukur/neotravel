...
export const HotelLarge = ({cardInfo, isLiked, isMinimal, onPress, onLikePress}) => {
    const item = cardInfo || {};
            ...
            <TouchableOpacity style={styles.container} onPress={onPress}>
                ...
                >
                    ...
                    <TouchableOpacity style={styles.heartHolder} onPress={onLikePress}>
                        <CustomSvg name={isLiked ? 'heartFull' : 'heartEmpty'} style={styles.heart}/>
                    </TouchableOpacity>
                ...
                {isMinimal ? null :
                 <View style={styles.cardInfo}>
                    <CustomText style={styles.loaction}>
                        {makeItShort(item.location, 30) || "~"}</CustomText>
                    <CustomText style={styles.description}>{makeItShort(item.description, 25) || "~"}</CustomText>
                    <CustomText style={styles.pricing}>{makeItShort(item.pricing, 20) || "~"}</CustomText>
                    <CustomText style={styles.price}>{makeItShort(item.currency, 3) || "$"} {item.price || "~"}</CustomText>
                </View>}
            </TouchableOpacity>
    );
};
